...
function SeatSelection() {
  ...
  const { movie, theatre, timing } = locationData.state;
  ...
  const fetchBookedSeats = async () => {
    try {
      // Only fetch seats for THIS specific movie + theatre + timing
  ...
  return (
    <div className="bg-black text-white min-h-screen px-10 py-10">
      ...
      {/* Loading */}
      {loading ? (
        <div className="text-center text-gray-400 text-xl">Loading seats...</div>
      ) : (
        <>
          {/* Seats */}
          ...
          <div className="max-w-xl mx-auto mt-14 bg-[#1a1a1a] p-8 rounded-2xl">
            <h2 className="text-3xl font-bold mb-5">Booking Summary</h2>

            <p className="mb-3">
              🎬 Movie: <span className="text-red-500 ml-2">{movie.title}</span>
            </p>
            <p className="mb-3">
              🏢 Theatre: <span className="text-red-500 ml-2">{theatre}</span>
            </p>
            <p className="mb-3">
              ⏰ Timing: <span className="text-red-500 ml-2">{timing}</span>
            </p>
            <p className="mb-3">
              💺 Seats:{" "}
            ...
            >
              {selectedSeats.length === 0 ? "Select at least 1 seat" : "Continue To Payment"}
            </button>
          </div>
        </>
      )}
